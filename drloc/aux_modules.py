...
def collect_samples_faster(feats, pxy, batch_size):
    n, c, h, w = feats.size()
    # [n, c, h, w] -> [n, c, hw] -> [c, nhw]
    feats = feats.view(n, c, -1).permute(1, 0, 2).reshape(c, -1)
    pxy = ((torch.arange(n).long().to(pxy.device) * h * w).view(n, 1) +
           pxy[:, :, 0]*h + pxy[:, :, 1]).view(-1)  # [n, m, 2] -> [nm]
...
def collect_positions(batch_size, N):
    ...
    rnd = torch.stack([torch.randperm(N*N)
                      for _ in range(batch_size)], dim=0)  # [B, N*N]
    pts_rnd = torch.stack([pts_norm[idx, r]
                          for idx, r in enumerate(rnd)], dim=0)  # [B, N*N, 2]
    return pts_norm, pts_rnd

# 필요한 feature 추가해주고, loss에서는 loss만 더하기
...
class DenseRelativeLoc(nn.Module):
    ...
    def forward_features(self, x):  # , mode="part"):
        ...
        # if mode == "part":
        # (B, sample size=32, 2) 640에서 sampling
        # (B, sample_size=32, 2) 192에서 sampling
        pxs = randn_sampling(H, self.sample_size, B).detach()
        pys = randn_sampling(W, self.sample_size, B).detach()

        deltaxy = (pxs-pys).float().to(x.device)  # [B, sample_size, 2]
        ptsx = collect_samples_faster(x, pxs, B).transpose(
            1, 2).contiguous()  # [B, sample_size, C]
        ptsy = collect_samples_faster(x, pys, B).transpose(
            1, 2).contiguous()  # [B, sample_size, C]
        ...
        #print(torch.cat([ptsx, ptsy], dim=2).shape)
        # ([4, 32, 192])
        # ([4, 32, 384])
        # ([4, 32, 768])
        ...
        # print('a')
        # pred_feats
        # deltaxy: (B, sample_size, 2)
        # H: int
        return pred_feats, deltaxy, H, W

    def forward(self, x, normalize=False):

        pred_feats, deltaxy, H, W = self.forward_features(x)
        ...
        if self.drloc_mode == "l1":
            # [B*sample_size, Output_size]
            predxy = pred_feats.view(-1, self.out_dim)
        else:
            predx, predy = self.unshared[0](
                pred_feats), self.unshared[1](pred_feats)
            # [B*sample_size, Output_size]
            predx = predx.view(-1, self.out_dim)
            # [B*sample_size, Output_size]
            predy = predy.view(-1, self.out_dim)
            # [B*sample_size, Output_size, 2]
            predxy = torch.stack([predx, predy], dim=2)
