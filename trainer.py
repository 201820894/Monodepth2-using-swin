...
class Trainer:
    ...
    def process_batch(self, inputs):
        ...
        features, drloc_list, deltaxy_list, plz_list = self.models["swin_encoder"](
            inputs[("color_aug", 0, 0)])

        #print("featureshape:", features.shape)

        # 여기서 처음으로 dictionary 형태의 outputs를 받음
        # 목표는 이 outputs에 key로 "drloc", "deltaxy", "plz"를 넣는거
        outputs = self.models["swin_decoder"](features)

        outputs["drloc"] = drloc_list
        outputs["deltaxy"] = deltaxy_list
        outputs["plz"] = plz_list
        if self.opt.predictive_mask:
    ...
    def compute_losses(self, inputs, outputs):
        ...
        loss_ssup, ssup_items = criterion_ssup(
            outputs, mode="l1", lambda_drloc=0.5)
        total_loss += loss_ssup
        print("self_supervised_loss: {}".format(loss_ssup))
