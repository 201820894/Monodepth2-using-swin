...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self,
        ...
        # Number of layers and features
        self.num_layers = len(depths)
        self.num_features = int(embedding_channels *
                                2 ** (self.num_layers - 1))
        ...
        for index, (depth, number_of_head) in enumerate(zip(depths, number_of_heads)): 
            self.stages.append(
                SwinTransformerStage(
                    ...
                    dropout_path=dropout_path[sum(
                        depths[:index]):sum(depths[:index + 1])],
