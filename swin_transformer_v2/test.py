...
__all__: List[str] = ["SwinTransformerStage",
                      "SwinTransformerBlock", "DeformableSwinTransformerBlock"]
...
def unfold(input: torch.Tensor,
    ...
    output: torch.Tensor = output.permute(
        0, 2, 3, 1, 5, 4).reshape(-1, channels, window_size[0], window_size[1])
...
def fold(input: torch.Tensor,
    ...
    batch_size: int = int(
        input.shape[0] // (height * width // window_size[0] // window_size[1]))
    # Reshape input to
...
class WindowMultiHeadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.meta_network: nn.Module = nn.Sequential(
            nn.Linear(in_features=2,
                      out_features=meta_network_hidden_features, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear(in_features=meta_network_hidden_features, out_features=number_of_heads, bias=True))
        # Init tau
        self.register_parameter("tau", torch.nn.Parameter(
            torch.ones(1, number_of_heads, 1, 1)))
    ...
    def __make_pair_wise_relative_positions(self):  # None
        ...
        """
        h_indexes: torch.Tensor = torch.arange(
            self.window_size[0], device=self.tau.device)
        w_indexes: torch.Tensor = torch.arange(
            self.window_size[1], device=self.tau.device)
        coordinates: torch.Tensor = torch.stack(
            torch.meshgrid([h_indexes, w_indexes]), dim=0)
        coordinates: torch.Tensor = torch.flatten(coordinates, start_dim=1)
        relative_coordinates: torch.Tensor = coordinates[:,
                                                         :, None] - coordinates[:, None, :]
        relative_coordinates: torch.Tensor = relative_coordinates.permute(
            1, 2, 0).reshape(-1, 2).float()
    ...
    def update_resolution(self,
                          new_window_size: tuple,
                          **kwargs: Any):  # None
        """
        Method updates the window size and so the pair-wise relative positions
        :param new_window_size: (tuple) New window size
        :param kwargs: (Any) Unused
        """
        # Set new window size
        self.window_size: tuple = new_window_size
    ...
    def __get_relative_positional_encodings(self):  # torch.Tensor
        ...
        relative_position_bias: torch.Tensor = relative_position_bias.permute(
            1, 0)
    ...
    def __self_attention(self,
        ...
        attention_map: torch.Tensor = torch.einsum("bhqd, bhkd -> bhqk", query, key) \
            / torch.maximum(torch.norm(query, dim=-1, keepdim=True)
                            * torch.norm(key, dim=-1,
                                         keepdim=True).transpose(-2, -1),
        ...
        output: torch.Tensor = torch.einsum(
            "bhal, bhlv -> bhav", attention_map, value)
        output: torch.Tensor = output.permute(
            0, 2, 1, 3).reshape(batch_size_windows, tokens, -1)
        return output
            ...
            attention_map: torch.Tensor = \
                ...
                / torch.maximum(torch.norm(query[:, :, token_index_query], dim=-1, keepdim=True)
                                * torch.norm(key, dim=-1, keepdim=False),
                                torch.tensor(1e-06, device=query.device, dtype=query.dtype))
            attention_map: torch.Tensor = attention_map / \
                self.tau.clamp(min=0.01)[..., 0]
            # Apply positional encodings
            attention_map: torch.Tensor = attention_map + \
                relative_position_bias[..., token_index_query, :]
            # Apply mask if utilized
            if mask is not None:
                number_of_windows: int = mask.shape[0]
    ...
    def forward(self,
        ...
        input: torch.Tensor = input.reshape(
            batch_size_windows, channels, tokens).permute(0, 2, 1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
                 ...
                 number_of_heads: int,
                 window_size: tuple = (6, 20),
                 shift_size: tuple = (3, 10),
        ...
        :param window_size: (tuple) Window size to be utilized
        :param shift_size: (tuple) Shifting size to be used
        ...
        self.normalization_1: nn.Module = nn.LayerNorm(
            normalized_shape=in_channels)
        self.normalization_2: nn.Module = nn.LayerNorm(
            normalized_shape=in_channels)
    ...
    def __make_attention_mask(self) -> None:
        ...
        if (self.shift_size[0] > 0 or self.shift_size[1] > 0):
            ...
            height_slices: Tuple = (slice(0, -self.window_size[0]),
                                    slice(-self.window_size[0], -
                                          self.shift_size[0]),
                                    slice(-self.shift_size[0], None))
            width_slices: Tuple = (slice(0, -self.window_size[1]),
            ...
            for height_slice in height_slices:
                for width_slice in width_slices:
                    ...
                    counter += 1
            mask_windows: torch.Tensor = unfold(
                mask[None, None], self.window_size)
    ...
    def update_resolution(self,
        ...
        self.window_attention.update_resolution(
    ...
    def forward(self,
        ...
        if (self.shift_size[0] > 0 or self.shift_size[1] > 0):
            output_shift: torch.Tensor = torch.roll(input=input, shifts=(-self.shift_size[0], -self.shift_size[1]),
                                                    dims=(-1, -2))
        else:
        ...
        if (self.shift_size[0] > 0 or self.shift_size[1] > 0):
            output_shift: torch.Tensor = torch.roll(input=output_merge, shifts=(self.shift_size[0], self.shift_size[1]),
                                                    dims=(-1, -2))
        else:
            output_shift: torch.Tensor = output_merge
        # Perform normalization
        output_normalize: torch.Tensor = self.normalization_1(
            output_shift.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        # Skip connection
...
class DeformableSwinTransformerBlock(SwinTransformerBlock):
    ...
    def __make_default_offsets(self) -> None:
        ...
        grid: torch.Tensor = torch.stack(
            torch.meshgrid([x, y])).transpose(1, 2)
    ...
    def update_resolution(self, new_window_size: tuple, new_input_resolution: Tuple[int, int]) -> None:
        """
        Method updates the window size and so the pair-wise relative positions
        :param new_window_size: (tuple) New window size
        :param new_input_resolution: (Tuple[int, int]) New input resolution
    ...
    def forward(self,
        ...
        offsets: torch.Tensor = offsets.reshape(
            batch_size, -1, 2, height, width).permute(0, 1, 3, 4, 2)
        ...
        offset_grid: torch.Tensor = self.default_grid.repeat_interleave(
            repeats=offsets.shape[0], dim=0) + offsets
...
class PatchMerging(nn.Module):
    ...
    def __init__(self,
        ...
        # Init normalization
        self.normalization: nn.Module = nn.LayerNorm(
            normalized_shape=4 * in_channels)
        # Init linear mapping
    ...
    def forward(self,
        ...
        # Unfold input
        input: torch.Tensor = input.unfold(
            dimension=1, size=2, step=2).unfold(dimension=2, size=2, step=2)
        input: torch.Tensor = input.reshape(
...
class SwinTransformerStage(nn.Module):
    ...
    def __init__(self,
        ...
        # Init downsampling
        self.downsample: nn.Module = PatchMerging(
            in_channels=in_channels) if downscale else nn.Identity()
        ...
        self.blocks: nn.ModuleList = nn.ModuleList([
            block(in_channels=in_channels,
                  ...
                  number_of_heads=number_of_heads,
                  window_size=window_size,
                  shift_size=(0, 0) if ((index % 2) == 0) else (
                      window_size[0]//2, window_size[1]//2),
                  ff_feature_ratio=ff_feature_ratio,
                  dropout=dropout,
                  dropout_attention=dropout_attention,
                  dropout_path=dropout_path[index] if isinstance(
                      dropout_path, list) else dropout_path,
                  sequential_self_attention=sequential_self_attention)
    ...
    def update_resolution(self, new_window_size: tuple, new_input_resolution: Tuple[int, int]) -> None:
        ...
        for block in self.blocks:  # type: SwinTransformerBlock
            block.update_resolution(
                new_window_size=new_window_size, new_input_resolution=self.input_resolution)
